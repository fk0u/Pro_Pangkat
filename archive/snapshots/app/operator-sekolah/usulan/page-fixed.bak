...
export default function OperatorSekolahUsulanPage() {
  ...
  const handleViewDocuments = (usulan: Usulan) => {
    const apiUsulan = usulanData.find(u => u.id === usulan.id)
    if (apiUsulan && apiUsulan.documents && apiUsulan.documents.length > 0) {
      setSelectedDocuments(apiUsulan.documents)
      setActiveProposalId(usulan.id)
      setIsViewDocumentsModalOpen(true)
    } else {
      toast({
        title: "Tidak ada dokumen",
        description: "Usulan ini belum memiliki dokumen yang diunggah",
        variant: "destructive"
      })
    }
  }
  ...
  const handleApproveDocument = async (documentId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Gagal",
          description: errorData.message || "Gagal menyetujui dokumen",
          variant: "destructive"
        })
      }
    } catch (error) {
      ...
      <div className="space-y-6">
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleViewDocuments(usulan)}
                            >
        ...
        <Dialog open={isViewDocumentsModalOpen} onOpenChange={setIsViewDocumentsModalOpen}>
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Dokumen Usulan Kenaikan Pangkat</DialogTitle>
              <DialogDescription>
                Dokumen yang diunggah untuk usulan {selectedUsulan?.nama}
              </DialogDescription>
            </DialogHeader>
            ...
            {selectedDocuments.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-sm text-muted-foreground">
                  Tidak ada dokumen yang diunggah untuk usulan ini.
                </p>
              </div>
            ) : (
              <div className="space-y-4 mt-4">
                {selectedDocuments.map((document) => (
                  <div key={document.id} className="p-4 bg-white rounded-lg shadow">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium">{document.nama}</p>
                        <p className="text-xs text-muted-foreground">
                          Diunggah pada {new Date(document.createdAt).toLocaleString('id-ID')}
                        </p>
                      </div>
                    ...
                    <div className="mt-4 flex gap-2">
                      <Button
                        ...
                        disabled={document.status === 'DISETUJUI'}
                      >
                        <CheckCircle className="h-4 w-4 mr-1" />
                        Setujui
                      </Button>
                      <Button
                        ...
                        disabled={document.status === 'DITOLAK'}
                      >
                        <XCircle className="h-4 w-4 mr-1" />
                        Tolak
                      </Button>
            ...
            <DialogFooter className="mt-6 pt-4 border-t border-gray-200">
              <Button 
                variant="outline" 
                onClick={() => setIsViewDocumentsModalOpen(false)}
                className="px-6 border-gray-300 text-gray-700 hover:bg-gray-50"
              >
                Tutup
              </Button>
