...
export default function RolePage() {
  ...
  const itemsPerPage = 15 // Limit users per page
  
  const { toast } = useToast()

  // Fetch roles and users with pagination
  const fetchRolesData = async (page: number = 1) => {
    try {
      setLoading(true)
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-8 space-y-6">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between py-4">
              <div className="text-sm text-muted-foreground">
                Menampilkan {Math.min((currentPage - 1) * itemsPerPage + 1, totalUsers)} -{" "}
              ...
              <div className="flex gap-2">
                ...
                >
                  <X className="h-4 w-4 mr-1" /> Sebelumnya
                </Button>
                <Button 
                  variant="outline" 
                  size="sm" 
                ...
                >
                  Selanjutnya <Check className="h-4 w-4 ml-1" />
                </Button>
              </div>
            </div>
          </div>
